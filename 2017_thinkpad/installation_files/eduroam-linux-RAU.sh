...
function setup_environment {
  ...
  URL="http://www.itc.rwth-aachen.de/servicedesk"
...
if ! ask "Dieses Installationsprogramm funktioniert nur für Anwender von ${bf}RWTH Aachen University.${n}" "Weiter" 1 ; then exit; fi
...
class EduroamNMConfigTool:
    ...
    def add_connection(self,ssid):
        server_alt_subject_name_list = dbus.Array({'DNS:radius.rz.rwth-aachen.de','DNS:radius1.rz.rwth-aachen.de','DNS:radius2.rz.rwth-aachen.de','DNS:radius3.rz.rwth-aachen.de'})
        server_name = 'rz.rwth-aachen.de'
        if self.nm_version == "0.9" or self.nm_version == "1.0":
...
network={
  ...
  identity="${USER_NAME}"
  domain_suffix_match="rz.rwth-aachen.de"
  phase2="auth=MSCHAPV2"
  password="${PASSWORD}"
}
EOFW
